...
import { FALLBACK_EXPERIENCE } from "@/src/lib/fallback-data";
...
const app = new Hono()
  .get("/", withHiddenAuth, async (c) => {
    ...
    try {
      ...
      data = await Experience.find(query).sort({ sortIndex: 1 });
    } catch (err) {
      console.warn("Database connection failed, using fallback experience:", err);
    }

    if (!data || data.length === 0) {
      data = FALLBACK_EXPERIENCE.filter((e) => c.req.query("withHidden") === "true" || !e.hide);
    }
