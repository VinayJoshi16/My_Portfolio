...
import { FALLBACK_STACK } from "@/src/lib/fallback-data";
...
const app = new Hono()
  .get("/", withHiddenAuth, async (c) => {
    ...
    try {
      ...
      console.warn("Database connection failed, using fallback stack:", err);
    }

    if (!data || data.length === 0) {
      const withHidden = c.req.query("withHidden") === "true";
      data = FALLBACK_STACK.map((group) => ({
        type: group.type,
        items: group.items.filter((item) => withHidden || !item.hide),
